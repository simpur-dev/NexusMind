...
const autoGeneratedRounds = computed(() => {
  ...
  if (!tc) {
    // 配置已加载但缺少 time_config，使用安全默认值
    return 40
  }
  const totalHours = tc.total_simulation_hours
  const minutesPerRound = tc.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return 40 // 配置数据不完整时使用默认值
  }
...
.step-card {
  ...
  border-radius: 16px;
  padding: 20px;
  box-shadow: 0 4px 16px rgba(15, 23, 42, 0.06), 0 1px 3px rgba(0, 0, 0, 0.03);
...
.step-card.active {
  ...
  box-shadow: 0 8px 24px rgba(37, 99, 235, 0.12), 0 0 0 1px rgba(59, 130, 246, 0.1);
}
...
.badge.success { background: rgba(34, 197, 94, 0.12); color: #15803d; border: 1px solid rgba(34, 197, 94, 0.2); }
.badge.warning { background: rgba(234, 179, 8, 0.12); color: #92600a; border: 1px solid rgba(234, 179, 8, 0.2); }
