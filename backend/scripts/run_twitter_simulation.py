"""
Compatibility wrapper for Twitter simulation.
Delegates execution to run_parallel_simulation.py with --twitter-only.
"""
...
HELP_TEXT = """usage: run_twitter_simulation.py --config CONFIG [--max-rounds N] [--start-round N] [--no-wait] [--no-world-model]

Compatibility wrapper around run_parallel_simulation.py --twitter-only mode.
...
def main():
    if any(arg in {"-h", "--help"} for arg in sys.argv[1:]):
        print(HELP_TEXT)
        return
    os.execv(sys.executable, _target_args("--twitter-only"))
